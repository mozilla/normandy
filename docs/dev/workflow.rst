...
-----------------------
Normandy uses hashed requirements for all of our dependencies. This works
differently in Python and JS.

Python
~~~~~~
Hashes for Python package are included in ``requirements/default.txt``. These
help verify that dependencies downloaded by pip haven't been tampered with.
...
JavaScript
~~~~~~~~~~
Hashes for JavaScript are stored in ``yarn.lock``. This file is automatically
generated by Yarn, and shouldn't be edited directly.

To add a new dependency, use ``yarn add``. For example, to add a package named
foo, use ``yarn add foo``. This will add foo to ``package.json``, and add foo
with all of its dependencies to ``yarn.lock``.
