...
class TestRecipeAPI(object):
    ...
    class TestCreation(object):
        ...
        def test_creation_when_action_does_not_exist(self, api_client):
            res = api_client.post(
                "/api/v2/recipe/", {"name": "Test Recipe", "action_id": 1234, "arguments": {}}
            )
        ...
        def test_creation_when_action_id_is_missing(self, api_client):
            res = api_client.post("/api/v2/recipe/", {"name": "Test Recipe", "arguments": {}})
            assert res.status_code == 400
            assert res.json()["action_id"] == [
    ...
    class TestUpdates(object):
        ...
        def test_it_can_change_arguments_for_recipes(self, api_client):
            ...
            res = api_client.patch(
                "/api/v2/recipe/%s/" % recipe.id, {"action_id": action.id, "arguments": arguments}
            )
    ...
    class TestFiltering(object):
        ...
        def test_search_works_with_arguments(self, api_client):
            ...
            res = api_client.get("/api/v2/recipe/?text=2")
            assert res.status_code == 200
        ...
        def test_search_out_of_order(self, api_client):
            ...
            res = api_client.get("/api/v2/recipe/?text=banana apple")
            assert res.status_code == 200
            assert [r["id"] for r in res.data["results"]] == [r1.id]

            res = api_client.get("/api/v2/recipe/?text=daikon cherry")
            assert res.status_code == 200
            assert [r["id"] for r in res.data["results"]] == [r2.id]
...
class TestApprovalRequestAPI(object):
    ...
    def test_approve(self, api_client):
        ...
        res = api_client.post(
            "/api/v2/approval_request/{}/approve/".format(a.id), {"comment": "r+"}
        )
        assert res.status_code == 200
    ...
    def test_reject(self, api_client):
        ...
        a = ApprovalRequestFactory(revision=r.latest_revision)
        res = api_client.post(
            "/api/v2/approval_request/{}/reject/".format(a.id), {"comment": "r-"}
        )
        assert res.status_code == 200
...
class TestApprovalFlow(object):
    ...
    def test_full_approval_flow(self, settings, api_client, mocked_autograph):
        ...
        res = api_client.post(
            "/api/v2/recipe_revision/{}/request_approval/".format(
        ...
        res = api_client.post(
            "/api/v2/approval_request/{}/approve/".format(approval_data["id"]), {"comment": "r+"}
        )
        ...
        api_client.force_authenticate(user1)
        res = api_client.patch(
            "/api/v2/recipe/{}/".format(recipe_data_1["id"]),
    ...
    def test_cancel_approval(self, api_client, mocked_autograph):
        ...
        api_client.force_authenticate(user2)
        res = api_client.post(
            f"/api/v2/approval_request/{approval_request_id}/approve/", {"comment": "r+"}
        )
        assert res.status_code == 200
        ...
        api_client.force_authenticate(user1)
        res = api_client.patch(
            f"/api/v2/recipe/{recipe_id}/", {"extra_filter_expression": "counter == 1"}
        )
        assert res.status_code == 200
        revision_id = res.json()["latest_revision"]["id"]
