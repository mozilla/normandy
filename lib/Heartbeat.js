...
exports.Heartbeat = class {
  constructor(chromeWindow, eventEmitter, options) {
    ...
    if (!options.sandbox) {
      throw new Error("sandbox must be provided");
    }
  ...
  maybeNotifyHeartbeat(name, data={}) {
    Log.debug("maybeNotifyHeartbeat");
    if (this.pingSent) {
    ...
    this.eventEmitter.emit(name, Cu.cloneInto(data, this.options.sandbox));
    ...
    this.eventEmitter.emit("TelemetrySent", Cu.cloneInto(payload, this.options.sandbox));
