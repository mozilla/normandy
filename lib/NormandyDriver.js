...
exports.NormandyDriver = function(sandbox, extraContext) {
  return {
    ...
    saveHeartbeatFlow(data) {
      const log = Log.makeNamespace('NormandyDriver.saveHeartbeatFlow');
      let url;
      if (!this.testing) {
      ...
      log.info(`sending heartbeat flow data to Input at ${url}`);

      // Make request to input
      let p = Http.post({url, data, headers});
      p.then(response => {
        log.info('Input response:');
        log.info(uneval(response.text));
      });
      p.catch(response => {
        log.error('Input response:');
        log.error(uneval(response.text));
      });
      // swallow output
      p = p.then(() => {});
    ...
    uuid() {
      const log = Log.makeNamespace('NormandyDriver.uuid');
