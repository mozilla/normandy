...
exports.NormandyDriver = function(sandbox, extraContext) {
  return {
    ...
    showHeartbeat(options) {
      ...
      sandbox._heartbeatEvents = Cu.cloneInto(events, sandbox, {cloneFunctions: true});

      let listenerScript = `
        addEventListener('UITour:SendPageNotification', message => { // eslint-disable-line no-undef
          Log.info('UITour:SendPageNotification', message);
          if (message.name !== 'UITour:SendPageNotification') {
            return;
          }
          let eventName = message.data.event.replace(/^Heartbeat:/, '');
          _heartbeatEvents.emit(eventName, message.data.params);
        });
      `;

      Cu.evalInSandbox(listenerScript, sandbox);

      // UITour is defined here: https://dxr.mozilla.org/mozilla-central/source/browser/components/uitour/UITour.jsm#1096
      UITour.showHeartbeat(browser, {
      ...
      return sandbox.Promise.resolve(sandbox._heartbeatEvents);
    },
