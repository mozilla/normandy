...
exports.NormandyDriver = function(sandbox, extraContext) {
  ...
  let heartbeatEventEmitters = {};

  browser.windowRoot.addMesageListener('UITour:SendPageNotification', function(message) {
    Log.info('UITour:SendPageNotification', message);
    // if (message.name !== 'UITour:SendPageNotification') {
    //   return;
    // }
    const eventEmitter = heartbeatEventEmitters[message.data.params.flowId];
    if (!eventEmitter) {
      return;
    }
    let eventName = message.data.event.replace(/^Heartbeat:/, '');
    eventEmitter.emit(eventName, message.data.params);
  });
  ...
  return {
    ...
    showHeartbeat(options) {
      ...
      heartbeatEventEmitters[options.flowId] = events;
