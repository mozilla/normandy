...
def test_approval_request(conf, requests_session, headers):
    ...
    response = requests_session.post(
        urljoin(conf.getoption("server"), "/api/v3/recipe_revision/{}/request_approval/".format(recipe_details['latest_revision_id'])),
        headers=headers
    )
    data = response.json()
    approval_id = data['id']
    ...
    response = requests_session.post(
        urljoin(conf.getoption("server"), "/api/v3/approval_request/{}/approve/".format(approval_id)),
        data={"comment": "r+"},
        headers=headers
    )
    assert response.status_code != 404
