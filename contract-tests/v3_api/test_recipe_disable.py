...
def test_recipe_disable(conf, requests_session, headers):
    ...
    response = create_approval_request(
        requests_session, conf.getoption("server"), recipe_details["latest_revision_id"], headers
    )
    data = response.json()
    approval_id = data["id"]
    ...
    response = approve_approval_request(
        requests_session, conf.getoption("server"), approval_id, headers
    )
    assert response.status_code != 404
    assert_valid_schema(response.json())
    ...
    response = enable_recipe(
        requests_session, conf.getoption("server"), recipe_details["id"], headers
    )
    assert response.status_code not in (404, 500)
    assert_valid_schema(response.json())
    ...
    response = requests_session.post(
        urljoin(
            conf.getoption("server"), "/api/v3/recipe/{}/disable/".format(recipe_details["id"])
        ),
        headers=headers,
    )
    assert response.status_code not in (404, 500)
