...
class TestRenderBundle(object):
    ...
    def mock_sri_hash(self, mocker):
        ...
        sri_hash.return_value = 'mock-hash'
        return sri_hash
    ...
    def test_html(self, mock_get_files, mock_sri_hash):
        ...
        script_tag = (
            '<script type="text/javascript" src="file.js" integrity="mock-hash" foo="bar">'
            '</script>'
        )
        assert script_tag in html

        link_tag = (
            '<link type="text/css" href="file.css" rel="stylesheet" integrity="mock-hash" '
            'foo="bar"/>'
        )
